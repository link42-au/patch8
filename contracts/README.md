...
Status: **P3c source-complete; no production dataset implemented**.
...
- [`data-content-v1.json`](data-content-v1.json) closes the v1 source, exact ordered field/type/nullability and table-key,
  ...
  [`docs/data-scope-v1.md`](../docs/data-scope-v1.md). Contract version 3 retains reusable source-specific CVE metadata,
  the exact CVSS-v2 severity path, and stable KEV identity while representing only actual ordered NVD nodes and removing
  nonexistent configuration-container source paths.
...
- [`fixtures/source-policy.cases.json`](fixtures/source-policy.cases.json) and
  ...
  fail-closed boundaries (24 source-policy cases and 59 manifest cases).
...
`3:3.0.0` by merely updating hashes or regenerating manifest schemas. The historical `1:1.0.0` and `2:2.0.0` baselines
remain immutable evidence and were not rewritten.
