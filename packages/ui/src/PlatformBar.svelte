...
interface PlatformApp {
	id: NonNullable<Props["currentApp"]>;
	label: string;
	href: string;
	color: string;
}

const APPS: readonly PlatformApp[] = [
	{
		id: "link42",
		label: "link42",
...
<div class="platform-bar">
  <nav class="pb-nav" aria-label="Platform navigation">
    {#each APPS as app}
      <a
        href={app.id === currentApp ? currentHref : app.href}
        class="pb-app"
      ...
      >
        ...
        {:else if app.id === "threat10"}
          <svg class="pb-icon" viewBox="0 0 120 120" width="28" height="28" aria-hidden="true">
            <circle cx="60" cy="60" r="20" fill="none" stroke="currentColor" stroke-width="5" />
            <line x1="74" y1="74" x2="86" y2="86" stroke={app.color} stroke-width="10" stroke-linecap="round" />
          </svg>
        {:else if app.id === "patch8"}
          <svg class="pb-icon" viewBox="0 0 120 120" width="28" height="28" aria-hidden="true">
            <path d="M 60 20 L 25 28 L 25 65 C 25 85 45 95 60 102 Z" fill="currentColor" opacity="0.5" />
            <path d="M 60 20 L 95 28 L 95 65 C 95 85 75 95 60 102 Z" fill={app.color} />
          </svg>
        {/if}
