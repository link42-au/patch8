...
The complete v1 implementation covers only redistribution-approved fields from these official sources. Five sources are
...
| CVE Program `cvelistV5` | Canonical CVE Record lineage and permitted CVE fields | **P3 policy registration required before any `cvelistV5` fetch or CVE Program-derived output.** Preserve the CVE/MITRE notice and record provenance; do not publish unidentified third-party material. |
...
Proceed strictly in feature order. P3a is the source-complete documentation prerequisite, not implementation evidence.
The next code work finishes P2 and implements P3. Official NVD/KEV, CVE Program, GHSA/CWE, and Vulnrichment ingestion
then proceeds through the bounded P4, P4a, P5, and P5a phases. Complete v1 local Parquet generation requires all phases;
HF publication begins only after that local build is complete and HF1 is resolved.
