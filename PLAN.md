...
| P3c | **Align contracts with the live NVD shape** | User approval, P3b | Contract 3 / policy 3.0.0 represents only actual NVD nodes: each top-level node has null parent, stable configuration/node identities, source order, operator, negate, and CPE children. Remove nonexistent configuration-container operator/negate paths. Pin timezone-less NVD timestamps as UTC, live `lastModStartDate` / `lastModEndDate` deltas, exact overlap, and mandatory periodic complete reconciliation because the live API rejects `includeMatchStringChange`. Preserve historical baselines, all non-NVD rules, descriptions-off, and no production data claim. Exact contracts, 24 policy fixtures, and 59 manifest fixtures pass. | source-complete | **source-complete** |
| P4 | **Build the official NVD and KEV core** | P3, P3b, P3c | A deterministic standard-library Python pipeline performs bounded NVD full imports and overlapping modified-window deltas plus complete CISA KEV snapshot reconciliation. It emits policy-approved source observations/provenance only; descriptions remain unavailable until P4a. Forty-one deterministic tests cover pre-allocation stream limits, redirect/final identity, total deadlines including remaining-time socket/wait caps, pagination/overshoot, zero-result live shape, sealed checkpoint forgery, both page-journal crash boundaries, watermark gaps/overlap and an enforced seven-day full-reconciliation clock, actual NVD nodes/ranges/CVSS, canonical reference/KEV lists and ordering, KEV reconciliation/stable identity, malformed data, throttling, atomic activation, and clean/delta equivalence. Live in-memory canaries validated the official NVD CVE, zero-result, and non-redirecting source identities without retaining source payloads. This intermediate core is not a v1 launch or production dataset. | source-complete | **source-complete** |
| P4a | **Add canonical CVE Program lineage** | P3, P3b, P3c, P4 | The standard-library builder consumes only the exact official `CVEProject/cvelistV5` codeload archive at an immutable 40-hex commit. Compressed/expanded/member/record/time limits apply to the complete archive before path filtering; exact origin/final URL, every member path/type/link target, schema versions, CVE/path identity, PUBLISHED state, CNA/assigner provider identity, full-record SHA-256, CVE/MITRE notice, sealed refetch-verified staging, and atomic activation fail closed. It emits source-specific CVE state/dates and every English CNA description while retaining a deterministic first-in-source-order pointer; safe ordinary metadata is ignored after validation and budgeting, while rejected records and all ADP, affected, reference, supporting-media, NVD-description, and other unregistered content emit nothing. Nine P4a fixture tests bring ingestion to 50 tests and cover malformed lineage/schema/path, commit/origin/redirect drift, complete-archive byte/member/type/link limits, safe ignored metadata, forged restart state, rejected records, stable observation identity, and byte-identical repeat builds. Official main resolved to `10c6b415a7a12a0c0fab006359939fcd34e2c78f`; exact-commit live shape canaries passed for representative 1999/2005/2020/2025/2026 records without retaining payloads. No full upstream build, Parquet, publication, or public dataset is claimed. | source-complete | **source-complete** |
| P5 | **Build CVE-linked GHSA package and MITRE CWE coverage** | P3, P3b, P3c, P4a | Ingest the public `github/advisory-database` repository and exact MITRE CWE archive at immutable versions. Advisory, aliases, affected packages, ranges, and ordered events remain separately keyed by GHSA/source indices. Only exactly-one-CVE-alias advisories project to per-CVE legacy package rows; multi-CVE observations remain advisory-associated and GHSA-only records never become CVEs. CWE enriches allowed source associations without creating vulnerability claims or scores. Fixtures cover single/multiple/no CVE aliases, cross-product rejection, repeated/disjoint ordered range events, withdrawals, ecosystem normalization, CWE relationships/history, conflicts, notices, and deterministic removals. | source-complete | planned |
| P5a | **Build CISA Vulnrichment observations** | P3, P3b, P3c, P4a | Add commit-pinned official CISA Vulnrichment input. Extract only CISA-ADP provider, SSVC, CVSS, CWE, and reference fields; keep observations source-specific and join by CVE without overwriting CNA/NVD/KEV/GHSA data. Fixtures cover additions, edits, removals, SSVC/CVSS/CWE lineage, conflicts, source revision changes, CNA-container rejection, and deterministic history. No raw mirror or EPSS-derived column is emitted. | source-complete | planned |
| P6 | **Generate query-routed Parquet releases locally** | P3b, P3c, P4, P4a, P5, P5a | Compile the complete v1 observations into the P2-budgeted table/partition/read-model layout with deterministic ordering and byte-stable output where the selected writer permits it. Validate schemas, routing coverage, uniqueness, bounds, rights and derivation provenance, source/capability states, representative legacy queries, no orphan rows, no blocked fields, 24-month normalized-history bounds, and no file-per-day accumulation. Establish and enforce numeric repository revision-count/retained-byte ceilings. Produce a local candidate manifest and previous-good rollback fixture without needing HF credentials. | source-complete | planned |
| P7 | **Publish the first verified HF dataset release** | P3b, P3c, P6, HF1 | Run the initial full build locally, upload candidate files to public `link42-au/patch`, read them back by immutable revision, run the release suite, and advance `manifest.json` in a second commit. Record exact data/manifest revisions, hashes, source watermarks, bytes, build time, query canary, and previous-good state. A failed check leaves no active release claim. | live-verified | **blocked: local full build and scoped publisher write/readback pending** |
| P8 | **Automate bounded daily source-change builds** | P3b, P3c, P7 | A public scheduled Action checks official source watermarks, exits without artifact churn when unchanged, and performs restartable deltas when changed. It enforces pacing, overlap, time/size/request budgets, deterministic tests, rights gates, candidate readback, manifest-last activation, and previous-good rollback. It overwrites bounded logical paths rather than adding dated daily files. A self-hosted run may be used when a full reconciliation exceeds public-runner budgets. | live-verified | planned |
| P9 | **Populate legacy CVE search, list, and detail views** | P3b, P3c, P7 | Existing legacy routes query the immutable release through the P2 client. Exact lookup, search/filter, pagination, CVSS/CWE/CPE, KEV, Vulnrichment, provenance, source timestamps, freshness, disabled EPSS, empty/error/stale, and previous-good states pass fixture and browser tests without UI redesign. Only minimal semantic copy/meta/disabled-control changes may replace false EPSS claims, with layout/visual fidelity preserved. | live-verified | planned |
| P10 | **Populate legacy product, package, report, and export views** | P3b, P3c, P9 | Existing views expose only coverage supported by the approved data and label limitations honestly. Product/vendor intelligence, recent activity, prioritisation, and CSV/JSON exports use pinned queried rows, preserve field provenance/freshness, escape untrusted data, and make no server or AI call. Partial package coverage and unavailable Patch Tuesday semantics replace false success only through minimal copy/meta/disabled-control changes with unchanged route/layout/visual fidelity. | live-verified | planned |
| P11 | **Add browser-local saved state** | P3b, P3c, P9 | Watchlists and saved searches are local, versioned, exportable/importable, recover from corrupt data, and never imply alerts, accounts, shared persistence, or cross-device synchronization. | live-verified | planned |
| P12 | **Cut over and operate the static product** | P3b, P3c, P8, P10, P11 | Production Pages canaries cover clean/warm cache, current and previous-good revisions, failed update, stale source, offline fallback, representative queries, desktop/mobile Safari, absence of backend/auth/secret calls, and legacy visual regressions. Releases record cost, Actions use, data age, transferred bytes, and rollback evidence. | live-verified | planned |
...
| P3c | P4 | The official live NVD 2.0 response/schema represents a configuration as a container with `nodes` only; `operator` and `negate` exist on each node, not on the configuration root. Contract 2 incorrectly required non-null direct-source root values. Live validation also established timezone-less UTC timestamps, current `lastMod*` parameters, and rejection of `includeMatchStringChange`. | P4, P4a, P5, P5a, P6, P7, P8, P9, P10, P11, P12 | **resolved: contract 3 / policy 3.0.0 source-complete** |
...
memory evidence; P3/P3b/P3c/P4 are source-complete independently and do not upgrade P2. The next data implementation is
the bounded P4a CVE Program phase, followed by P5 and P5a. Complete v1 local Parquet generation requires all
