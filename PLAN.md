...
| P4 | **Build the official NVD and KEV core** | P3, P3b, P3c | A deterministic standard-library Python pipeline performs bounded NVD full imports and overlapping modified-window deltas plus complete CISA KEV snapshot reconciliation. It emits policy-approved source observations/provenance only; descriptions remain unavailable until P4a. Thirty-one deterministic tests cover pagination, zero-result live shape, cross-page duplication, watermark gaps/overlap, actual NVD nodes/operators/negation/range bounds, all CVSS observations and deterministic selection, KEV adds/edits/removals/unknown state and stable identity, malformed data, throttling, restart/atomic activation, repeated complete reconciliation, and clean-build/delta equivalence. Live in-memory canaries validated the official NVD CVE and zero-result envelopes without retaining source payloads. This intermediate core is not a v1 launch or production dataset. | source-complete | **source-complete** |
